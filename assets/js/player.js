...
function initPlayer(tracks = window.defaultTracks) {
    ...
    window.audio = new Audio(tracks[currentTrack].src);
    ...
    function loadTrack(index, animate = true) {
        ...
        window.audio.onloadedmetadata = () => {
            let min = Math.floor(window.audio.duration / 60);
            let sec = Math.floor(window.audio.duration % 60).toString().padStart(2, "0");
    ...
    function updateProgress() {
        if (!isNaN(window.audio.duration)) {
            progressBar.style.width = `${(window.audio.currentTime / window.audio.duration) * 100}%`;
        }
        let min = Math.floor(window.audio.currentTime / 60);
        let sec = Math.floor(window.audio.currentTime % 60).toString().padStart(2, '0');
    ...
    function seekTrack(e) {
        ...
        if (!isNaN(window.audio.duration)) {
            window.audio.currentTime = (clickX / width) * window.audio.duration;
        }
    }
    ...
    window.audio.addEventListener("timeupdate", updateProgress);
